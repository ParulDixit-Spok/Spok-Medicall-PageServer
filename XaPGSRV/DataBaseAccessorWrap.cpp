...
#include "DataBaseAccessorWrap.h"
#include "Util2.h"
...
void DataBaseAccessor::ADORecordsetClose(void)
{
	try
	{
...
void DataBaseAccessor::SetConnectString(CString csConnectString)
{
...
void DataBaseAccessor::SetCommandTimeout(long Timeout)
{
	// connect string saved in a member variable,
	//  used for connect, and re-connect, also
	//  GetConnectString();
...
CString DataBaseAccessor::GetConnectString(void)
{
	// connect string saved in a member variable,
	//  used for connect, and re-connect, also
	//  GetConnectString();
...
int DataBaseAccessor::ADOConnectionConnect(void)
{
	int ReturnCode = FAIL;
	...
	try
	{
		adoConnection = new AdoWrapConnection(m_csConnectString, "", "");
...
int DataBaseAccessor::ADORsSelect(CString csCaller,
								CString csTable,
								CString csSelectStatement)
{
	...
	try{
	auto_ptr<AdoWrapCommand> cmdPtr(new AdoWrapCommand(adoConnection));
	auto_ptr<AdoWrapRecordSet> recPtr(cmdPtr->Execute(csSelectStatement));
	}
	catch()
	{
	}
...
DataBaseAccessor::ADORsCommand(CString csCaller,
	...
	try
		{
		// convert to "C" character array string.
		sprintf(CommandStatement, "%s", csCommandStatement);
